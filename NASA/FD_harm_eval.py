...
#   Plots the resulting spectra in dB
    if len(mics)>1:
        ax[i].plot(f_ms,10*np.log10(Gxx_avg_ms*df/20e-6**2),linestyle='-')
        ...
        ax.plot(f,10*np.log10(Gxx_avg_ms*df/20e-6**2),'-')
        ax.plot(f_bb,10*np.log10(Gxx_avg_bb*df/20e-6**2),linestyle='-.')
...
if len(mics)>1:
    ...
    ax[-1].legend(['Narrowband', 'Broadband','Tonal'], loc='center', ncol=3,bbox_to_anchor=(.5, -.65))
...
#   Plots the resulting spectra in dB
    if len(mics)>1:
        ...
        ax[i].plot(t_rev/(rpm_nom1/60)**-1,Xn_avg_filt[:,m-1])

    else:
        ax.plot(t_rev/(rpm_nom1/60)**-1,Xn_avg[:,m-1]-np.mean(Xn_avg[:,m-1]))
