...
li = -np.array([Fx[:,:-1]*np.sin(psi),-Fx[:,:-1]*np.cos(psi),Fz[:,:-1]])

dD_dt = np.diff(Fx)/dt
dT_dt = np.diff(Fz)/dt

li_dt =  -np.array([(dD_dt * np.sin(psi) + Fx[:,:-1] * loadParams['omega'] * np.cos(psi)), -dD_dt * np.cos(psi) + Fx[:,:-1] * loadParams['omega'] * np.sin(psi), dT_dt])
...
Mr_r,dMr_dt_r,lr_r, dlr_dt_r=list(map(lambda f: np.array([np.array([np.interp(xp=psi*180/np.pi,fp=f[m_itr,r_ind],x =tr[r_ind]) for r_ind in range(len(geomParams['rdim']))]) for m_itr,tr in enumerate(t_ret_ind)]),[Mr,dMr_dt,lr,dlr_dt]))
...
if save_h5:
    sdata = {'phi':phi,'dt':dt,'x':x,'y':y,'r':r,'ts':ts,'tr':t_ret,'tr_ind':t_ret_ind,'Mr_r':Mr_r,'dMr_dt_r':dMr_dt_r,'lr_r':lr_r,'dlr_dt_r':dlr_dt_r,'term1':term1,'term2':term2,'p_total':p_tot,'OASPL':OASPL}
