...
def eval_pressure(f):
    ...
    p = np.trapz(f,axis =1,x = np.repeat(abs(np.diff(surfNodes[:26,0])),geomParams['nXsecs'],axis = 0)*dr)
    ...
    p_tot = (4*np.pi)**-1*np.gradient(np.array([np.concatenate((p_tot[:,:-1][:, int(360 / UserIn['Nb']*Nb):], p_tot[:,:-1][:,:int(360 / UserIn['Nb']*Nb)]), axis=1) for Nb in range(UserIn['Nb'])]),axis = -1,edge_order=2)/dt
...
def linterp_ind(ind,quant):
    ...
    y0,y1 = list(map(lambda x: np.array([quant[m_ind,pnt,x[m_ind,pnt]] for pnt in range(np.shape(quant)[1]) for m_ind in range(len(quant))]).reshape((np.shape(quant)),order = 'F'), [x0,x1]))
...
surfNorms = geomParams['surfNorms']/np.expand_dims(np.linalg.norm(geomParams['surfNorms'],axis = 1),axis = 1)

#%%
v = loadParams['omega']*surfNodes[:,1]
vn = -surfNorms[:,0]*v
...
# wrap.plot()
# surfNorms_exp = np.array((np.expand_dims(surfNorms[:, 1], axis=-1) * np.cos(psi) + np.expand_dims(surfNorms[:, 0],axis=-1) * np.sin(psi),
...
y = np.array((np.expand_dims(surfNodes[:, 1], axis=-1) * np.cos(psi) + np.expand_dims(surfNodes[:, 0],axis=-1) * np.sin(psi),
              ...
              np.expand_dims(surfNodes[:, -1], axis=-1) * np.ones(len(psi))))
...
r = np.expand_dims(np.expand_dims(x.transpose(),axis = 2),axis = 3)-y
...
#
r_r,Mi_r = list(map(lambda f: linterp_ind(t_ret_ind,f), [r.transpose(0,2,3,1),np.repeat(np.expand_dims(Mi,axis = 0),31,axis = 0).transpose(0,2,3,1)]))
r_r_mag =  np.linalg.norm(r_r, axis=-1)
...
if save_h5:
    sdata = {'phi':phi,'dt':dt,'x':x,'y':y,'r':r,'ts':ts,'tr':t_ret,'tr_ind':t_ret_ind,'r_r':r_r,'Mr_r':Mr_r,'vn_r':vn_r,'p':p,'OASPL':OASPL}
