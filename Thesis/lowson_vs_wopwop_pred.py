...
#   Plots the resulting spectra in dB
        if len(mics)>1:
            ax[i].plot(lowson_dat['ts']/(lowson_dat['dt']*360), lowson_dat['p_total'][m-1])
            ...
            ax.plot(lowson_dat['ts']/(lowson_dat['dt']*360), lowson_dat['p_total'][m-1])
...
ax[-1].legend(["Lowson's LN",'WOPWOP'], ncol=2,loc='center',bbox_to_anchor=(0.5, -0.55))


#%%

fig, ax = plt.subplots(1, 1, figsize=(6.4, 4.5),subplot_kw=dict(polar=True))
ax.plot(lowson_dat['phi'], lowson_dat['OASPL'])
...
ax.legend(["Lowson's LN",'WOPWOP'], ncol=1,loc='center',bbox_to_anchor=(-0.05, 0.9))
